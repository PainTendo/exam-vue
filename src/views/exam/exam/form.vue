      ...
      <div>
        ...
        >
          ...
          >
            ...
            <template slot-scope="scope">
              <el-input-number v-model="scope.row.saqCount" :min="0" :max="scope.row.totalSaq" :controls="false" style="width: 100px" />  / {{ scope.row.totalSaq }}
            </template>
    ...
    <el-card style="margin-top: 20px">
      ...
      <el-form ref="postForm" :model="postForm" :rules="rules" label-position="left" label-width="120px">
        ...
        <el-form-item label="绑定课程">
          <el-select v-model="postForm.courses" filterable clearable placeholder="请选择课程" style="width: 100%">
          <el-option
            v-for="item in courses"
            :key="item.id"
            :label="item.subjectName+item.coursesCode+item.coursesName+item.coursesGrade"
            :value="item.id">
          </el-option>
        </el-select>
        </el-form-item>

        <el-form-item label="通过后生成证书">
          <el-switch
            v-model="postForm.createCertificate"
            active-color="#13ce66"
            :active-value="1"
            :inactive-value="0">
          </el-switch>
